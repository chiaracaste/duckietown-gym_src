...
class LineDetectorNode(object):
    ...
    def __init__(self):
        ...
        # Subscribers
        self.sub = rospy.Subscriber("/default/ground_projection/lineseglist_out", SegmentList, self.processSegments, queue_size=1)
        self.sub_velocity = rospy.Subscriber("/default/car_cmd_switch_node/cmd", Twist2DStamped, self.updateVelocity)
        self.sub_change_params = rospy.Subscriber("/default/lane_filter_node/change_params", String, self.cbChangeParams)
        # Publishers
        self.pub_lane_pose = rospy.Publisher("/default/lane_filter_node/lane_pose", LanePose, queue_size=1)
        self.pub_belief_img = rospy.Publisher("/default/lane_filter_node/belief_img", Image, queue_size=1)
        self.pub_seglist_filtered = rospy.Publisher("/default/lane_filter_node/seglist_filtered",SegmentList, queue_size=1)

        self.pub_ml_img = rospy.Publisher("/default/lane_filter_node/ml_img", Image, queue_size=1)


        self.pub_entropy    = rospy.Publisher("/default/lane_filter_node/entropy",Float32, queue_size=1)

        self.pub_parking_detection = rospy.Publisher("~parking_line", BoolStamped, queue_size=1)

        self.pub_parking_on = rospy.Publisher("/default/parking_on", BoolStamped,queue_size=1 )

        self.pub_exit_from_parking = rospy.Publisher("~exit_from_parking" , BoolStamped,queue_size=1)

        # FSM
        self.sub_switch = rospy.Subscriber("~switch",BoolStamped, self.cbSwitch, queue_size=1)
        self.sub_fsm_mode = rospy.Subscriber("/default/fsm_node/mode", FSMState, self.cbMode, queue_size=1)
    ...
    def cbMode(self, msg):
        if msg.state == "SLIM_PARKING":
            rospy.set_param('/default/lane_controller_node/v_bar', 0.14)
            rospy.set_param('/default/lane_controller_node/k_theta', -0.5)
            rospy.set_param('/default/stop_line_filter_node/stop_distance', 0.15)
        ...
        if msg.state == "EXITING_FROM_PARKING":
            rospy.set_param('/default/lane_controller_node/v_bar', -0.05)
            rospy.set_param('/default/lane_controller_node/k_theta', -0.04)
        if msg.state == "LANE_FOLLOWING":
            rospy.set_param('/default/lane_controller_node/v_bar', 0.23)
            rospy.set_param('/default/lane_controller_node/k_theta', -1)
            rospy.set_param('/default/stop_line_filter_node/stop_distance', 0.22)
