...
class LineDetectorHistogram(Configurable, LineDetectorInterface):
    #"""LaneFilterHistogram"""
    ...
    def getStatus(self):
        return LineDetectorInterface.GOOD
    ...
    def prepareSegments(self, segments):
        ...
        for segment in segments:
            ...
            if segment.color == segment.WHITE and self.isRightSegment(segment): # da sistemare
                self.inLane = True
    ...
    def generateVote(self, segment):
        ...
        # HO TOLTO IL CONTROLLO SE IL SEGMENTO E BLU, GIUSTO?

        if (p2[0] > p1[0]):  # left edge of blue line
            d_i = -d_i + self.linewidth_blue/2
            phi_i = -phi_i

        else:  # right edge of blue line
            d_i = d_i - self.linewidth_blue/2
    ...
    def isRightSegment(self,segment):
        x_c = (segment.points[0].x + segment.points[1].x) / 2
        y_c = (segment.points[0].y + segment.points[1].y) / 2
        return x_c < 0.2 and y_c > 0.1
    ...
    def getSegmentDistance(self, segment):
        x_c = (segment.points[0].x + segment.points[1].x) / 2
        y_c = (segment.points[0].y + segment.points[1].y) / 2
        if x_c < 0.2 and segment.color == segment.BLUE:
