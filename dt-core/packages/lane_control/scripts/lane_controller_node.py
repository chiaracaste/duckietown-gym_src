...
class lane_controller(object):
    ...
    def setGains(self):
        ...
        self.k_Dd = self.setupParameter("~k_Dd", k_Dd_fallback)  # gain for integrator of d
        self.k_Dphi = self.setupParameter("~k_Dphi", k_Dphi_fallback)  # gain for integrator of phi (phi = theta)
    ...
    def cbMode(self,fsm_state_msg):
        ...
        if self.fsm_state == "ARRIVE_AT_STOP_LINE" or "PARKED":
            self.sendStop()
    ...
    def updatePose(self, pose_msg):
        ...
        if not self.fsm_state == "SAFE_JOYSTICK_CONTROL":
            ...
            #if self.last_ms is not None:
                #omega += 0.01 * (0.22 / self.v_bar) * -self.cross_track_err / dt
                #omega += 0.01 * (0.22 / self.v_bar) * -self.heading_err / dt
                #print('ciaooooo')
                #print(self.k_Dd * (0.22 / self.v_bar) * -self.cross_track_err self.cr/ dt)
                #print(self.k_Dphi * (0.22 / self.v_bar) * -self.heading_err / dt)
        if car_control_msg.v == 0:
            omega = 0
        else:
         #check if velocity is large enough such that car can actually execute desired omega
            if car_control_msg.v - 0.5 * math.fabs(omega) * 0.1 < 0.065 and self.fsm_state != "EXITING_FROM_PARKING" and self.fsm_state != "SLIM_PARKING":
